...
async function computePreviewOnPage(code, def, preserveRest, selectedDate) {
  ...
  const resp = await chrome.scripting.executeScript({
    ...
    func: (selector, code, def, preserveRest, selectedDate, allCodes, monthNamesArray) => {
      ...
      // Remove any leading separators from rest and any trailing NC date tokens to avoid duplication
      rest = rest.replace(/^[\s\-:\|–—]+/, '').replace(/\|\s*NC:.*$/i, '').trim();

      let newValue;
      if (preserveRest) {
        newValue = code + (rest ? ' | ' + rest : '');
        if (!m && current.trim()) {
          // No leading code detected - treat full current as the rest but strip existing NC tokens
          const sanitized = current.replace(/\|\s*NC:.*$/i, '').trim();
          newValue = code + (sanitized ? ' | ' + sanitized : '');
        }
      } else {
        newValue = code;
      }
      
      // Add date if selected (always append after everything, using ' | NC: ')
      if (selectedDate) {
